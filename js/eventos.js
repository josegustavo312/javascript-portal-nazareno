...
function uploadFile(){
	...
	if(validarCampo()){
		if (confirmar("Salvar")){
			...
			var postKey = firebase.database().ref('eventos/').push().key; // obtem a chave do banco de dados
			
			var storageRef = firebase.storage().ref('Eventos/' + postKey); // salva a imagem com o nome da chave
			...
			uploadTask.on('state_changed', function(snapshot){
			  ...
			  updates['/eventos/' + postKey] = postData;
...
function queryDatabase(){
	...
	firebase.database().ref('/eventos/').limitToLast(qtdLimite).once('value').then(function(snapshot){
		...
		for(var i = inicioI; i < keys.length; i++){ // separa as chave 
			...
			$("#conteudoEventos").append(currentRow);
...
function urlStorage(key){
	...
	var storageRef = storage.ref('Eventos');
...
function removerDados(key){
	...
	if (confirmar("Excluir")){
		var storage = firebase.storage();
		var storageRef = storage.ref('Eventos/'+key);
			
		// Exclui os dados e imagem
		storageRef.delete().then(function() {
		  //remove os dados
		  firebase.database().ref().child('eventos').child(key).remove(); 
		  // remove a linha
		  $('.row'+key).remove();
